...
IOException
import java.util.*; // Optional for Scanner
...
if (version < 2) {
throw new InvalidConfigVersionException("Config version
too old!");
}
...
if (!pathFile.exists()) {
throw new IOException("Configured file path does not
exist: " + filePath);
}
...
System.out.println("Error: The first line of config.txt must be
a number.");
} catch (IOException e) {
System.out.println("Error: " + e.getMessage());
} catch (Exception e) {
